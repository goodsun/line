...
LINEミニアプリ（LIFF）と Messaging API を組み合わせた、PHP製のLINE連携スターターキット。ユーザー認証から、メッセージ送信・自動応答ボット・リッチメニューまで、LINE連携でよく使う機能を一通り実装済み。コピーして自分のサービスに組み込める。

## 主な機能

- **ミニアプリ認証**: LIFFでログイン → IDトークンをサーバー検証し、改ざんできない `userId` を取得
- **プロフィール取得**: 表示名・プロフィール画像、（権限申請後は）メールアドレス
- **同意フロー**: メール取得の通知・同意画面 ＋ プライバシーポリシーページ
- **プッシュ送信**: 検証済みuserId宛にメッセージ送信
- **自動応答ボット**: Webhook受信（署名検証付き）でテキストに自動応答
- **一斉配信**: 友だち全員へのブロードキャスト（CLI）
- **リッチメニュー**: トーク下部メニューの作成・適用（CLI）

## サンプルを試す

下のQRコードをLINEで読み取って友だち追加すると、サンプルを実際に試せます。

<img src="img/qr.jpg" alt="友だち追加用QRコード" width="240">

友だち追加後、トーク下部のリッチメニュー **右半分「アプリを開く」** からミニアプリを起動できます。左半分「お問い合わせ」でボットの自動応答も確認できます。
...
- **左半分**: 「お問い合わせ」メッセージを送信 → `webhook.php` が案内文を返信
- **右半分**: ミニアプリ（`https://miniapp.line.me/<LIFF_ID>`）を開く

画像は **2500x843**（compactサイズ）。同梱の `img/menu.jpg` が仮メニュー画像。

**前提**: `.env` に `LINE_CHANNEL_ACCESS_TOKEN` が設定済みであること（未設定だとAPIを叩けず失敗する）。
...
1. リッチメニュー作成（サイズ・2領域・アクションを登録）→ `richMenuId` を取得
2. 画像アップロード（`img/menu.jpg`）
3. 全ユーザーのデフォルトメニューに設定

> ⚠️ 手順3は **公式アカウントの全ユーザーに即時反映**される。実行前に内容を確認すること。
> 別画像に差し替えるときは再実行すればよい（新しいrichMenuIdが作られ、デフォルトが上書きされる）。古いメニューは `DELETE /v2/bot/richmenu/{richMenuId}` で削除できる。
